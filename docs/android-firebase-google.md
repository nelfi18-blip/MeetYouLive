...
- Android native plugin `webClientId`: derived from the existing `GOOGLE_CLIENT_ID` during the frontend build.
...
The Android Capacitor APK uses the `@capgo/capacitor-social-login` plugin (`SocialLogin.initialize()` / `SocialLogin.login()`) for the Google provider. The native flow obtains a Google ID token, sends it to `POST /api/auth/google-native`, and the backend verifies the token before issuing the MeetYouLive backend JWT.
