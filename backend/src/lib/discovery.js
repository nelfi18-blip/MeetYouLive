...
const buildLocationTextMatch = (viewer = {}) => {
  ...
  if (country) {
    const clauses = [{ "location.country": new RegExp(`^${escapeRegex(country)}$`, "i") }];
    if (city) {
      clauses.push({ "location.city": new RegExp(`^${escapeRegex(city)}$`, "i") });
    }
    return city ? { $and: clauses } : clauses[0];
  }
  if (legacyLabel) {
    return { locationLabel: new RegExp(escapeRegex(legacyLabel), "i") };
  }
  return null;
};
