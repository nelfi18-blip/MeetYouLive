...
export default function ReferralPage() {
  ...
  const fetchReferral = useCallback(async () => {
    setLoading(true);
    const [json, invJson] = await Promise.all([
      apiFetch("/api/referral/me"),
      apiFetch("/api/referral/invites"),
    ]);
    setData(json);
    if (invJson?.invites) setInvites(invJson.invites);
    setLoading(false);
  }, []);
  ...
  const pendingInvites = (invites || []).filter((i) => !i.rewardClaimed);
  const completedInvites = (invites || []).filter((i) => i.rewardClaimed);
        ...
        {data?.referralCode ? (
          <div className="link-card">
            ...
            <div className="share-row">
              {canShare && (
                <button className="btn-share btn-native" onClick={handleShare}>
...
const pageStyles = `
  ...
  .stat-label { font-size: 0.7rem; color: var(--text-muted, #94a3b8); text-align: center; }
  ...
  .btn-native { background: linear-gradient(135deg, #8b5cf6, #e040fb); color: #fff; }
  ...
  .invite-pending { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); }
  .invite-done { background: rgba(52,211,153,0.05); border: 1px solid rgba(52,211,153,0.15); }
  .invite-avatar {
  ...
  .invite-avatar-done { background: linear-gradient(135deg, #34d399, #10b981); }
  .invite-name {
    flex: 1;
