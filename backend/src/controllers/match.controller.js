...
exports.likeUser = async (req, res) => {
  ...
  try {
    ...
    trackMilestoneEvent("first_like", String(req.userId));
    if (mutual) trackMilestoneEvent("first_match", String(req.userId));
  } catch (err) {
    if (err.code === 11000) {
      let mutual;
      try {
...
exports.unlikeUser = async (req, res) => {
  ...
  try {
    ...
    await Promise.all([
      Like.deleteOne({ from: req.userId, to: userId }),
      Dislike.deleteOne({ from: req.userId, to: userId }),
    ]);
